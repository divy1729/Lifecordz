...
const DonorChatbot = ({ dialogMode }) => {
  ...
  // ✅ Updated to avoid duplicate path issues
  const getApiUrl = () => process.env.REACT_APP_API_URL || '/api/ai/donor-query';
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await axios.post(apiUrl, { question: input, history });

      setMessages(msgs => [...msgs, { sender: 'bot', text: res.data.answer }]);
    } catch (err) {
      console.error('Error in chatbot:', err);
      setMessages(msgs => [...msgs, { sender: 'bot', text: 'Sorry, I could not get an answer.' }]);
    }
    setInput('');
    setLoading(false);
  };
